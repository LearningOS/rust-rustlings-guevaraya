...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let arr_color = arr.iter().filter(|x | ** x >=0 && **x < 255 ).count();
        match arr_color{
            3 => Ok(Color{red:arr[0] as u8,green:arr[1] as u8, blue:arr[2] as u8}),
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        let color_cnt = slice.iter().filter(|x|**x >=0 && **x <= 255 ).count();
        if color_cnt == 3{
